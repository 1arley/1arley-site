...
/**
 * GRAVITY STARS BACKGROUND (from animate-ui, adapted).
 * A monochrome dust field that swirls around the cursor. Pauses off-screen,
 * caps DPR, respects prefers-reduced-motion (single static frame).
 */
function GravityStarsBackground({
  ...
  React.useEffect(() => {
    ...
    // Pause off-screen (and stop after one static frame under reduced motion)
    let visible = true;
    let iv: IntersectionObserver | null = null;
    if (typeof IntersectionObserver !== "undefined" && container) {
      iv = new IntersectionObserver((entries) => {
        visible = entries[entries.length - 1]?.isIntersecting ?? true;
        if (!visible && animRef.current) {
          cancelAnimationFrame(animRef.current);
          animRef.current = null;
        } else if (visible && animRef.current === null && !reducedRef.current) {
          animRef.current = requestAnimationFrame(animate);
        }
      });
  ...
  }, [resizeCanvas, animate, drawStars]);
